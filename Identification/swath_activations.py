...
def get_neighboring_pixel(img, x, y, current_window_size):
    ...
    for _ in range(math.ceil(max_num_tries / max_tries_per_neighbourhood)):
        for _ in range(max_tries_per_neighbourhood):
            ...
            max_x = min(img.shape[0], x + current_window_size)  # Use image height
            min_y = max(0, y - current_window_size)
            max_y = min(img.shape[1], y + current_window_size)  # Use image width
            
            x_rand = random.randint(min_x, max_x - 1)
            y_rand = random.randint(min_y, max_y - 1)
            total_tries += 1

            # Check if the randomly selected pixel is not missing (not black)
            if img[x_rand, y_rand] != 0:
                return x_rand, y_rand

        # Expand search window if no valid pixel is found
        current_window_size += neighbourhood_size_increment

    return x_rand, y_rand  # Return last attempt if no valid pixel found

def fill_swath_with_neighboring_pixel(img, left=10, right=100, top=10, bottom=100, color=0, current_window_size=10):
    """ 
    ...
    # Find all pixels that match the "missing" color (0 for black)
    x_swath, y_swath = np.where(img == color)

    for i in range(len(x_swath)):
        x_rand, y_rand = get_neighboring_pixel(img, x_swath[i], y_swath[i], current_window_size)

        # Ensure pixel is within the specified bounding box
        if left <= x_swath[i] <= right and top <= y_swath[i] <= bottom:
